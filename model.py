...
class SEGAN(Model):
    ...
    def __init__(self, sess, args, devices, infer=False, name='SEGAN'):
        ...
        self.g_enc_depths = [4,8,16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 1024,256+self.accent_class]
        self.pg_enc_depths = [1024, 512, 512, 256, 256, 128, 128, 64, 64]
        # Define D fmaps
        self.d_num_fmaps = [4,8,16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 512, 1024,256, 128]
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        print(wavbatch.shape)
        print(noisybatch.shape)
        ...
        if gpu_idx == 0:
            ...
            dummy_joint = wavbatch
            dummy = discriminator(self, dummy_joint, zvalue=class_val,
                                  reuse=False)

        G, zclass, ztot  = self.generator(noisybatch, is_ref=False, spk=None,
                               do_prelu=do_prelu)
        zclass = tf.nn.softmax(zclass)
        self.Gs.append(G)
        self.zs.append(ztot)
        ...
        dl_fk_logits = ClassDiscriminator(zclass)
        ...
        self.hist_sum["d_fk"] = histogram_summary("d_fake", d_fk_logits)
        self.hist_sum["dl_fk"] = histogram_summary("d_latent_class", dl_fk_logits)
        ...
        self.audio_sum["gen"] = audio_summary('G_audio', G)
        self.hist_sum["gen"] = histogram_summary('G_wav', G)

        if gpu_idx == 0:
            self.losses = {}
            self.loss = {}
            self.losses["g_loss"] = []
            self.losses["g_l1_loss"] = []
            self.losses["g_adv_loss"] = []
            self.losses["d_rl_loss"] = []
            self.losses["d_fk_loss"] = []
            self.losses["d_c_loss"] = []
            #self.d_nfk_loss = []
            self.losses["d_loss"] = []
            self.losses["dl_fk_loss"] = []
            self.losses["dl_r1_loss"] = []
            self.losses["dl_r2_loss"] = []
            self.losses["dl_disc_loss"] = []
            self.losses["dl_gen_loss"] = []
            self.losses["de_r1_loss"] = []
            self.losses["de_fk_loss"] = []
            self.losses["de_disc_loss"] = []
            self.losses["de_gen_loss"] = []
            self.losses["e_loss"] = []
            self.losses["dc_loss"] = []
            self.losses["dl_loss"] = []
        ...
        self.loss["g_l1_loss"] = self.l1_lambda * tf.reduce_mean(tf.abs(tf.subtract(G,
                                                                  wavbatch)))

        self.loss["g_loss"] = self.loss["g_adv_loss"] + self.loss["g_l1_loss"]
        # class_losses
        self.loss["dl_fk_loss"] = tf.reduce_mean(tf.squared_difference(dl_fk_logits, 0.))
        ...
        self.loss["dl_gen_loss"] = tf.reduce_mean(tf.squared_difference(dl_fk_logits,1.))
        # embedding-losses
            ...
            while not coord.should_stop():
                ...
                if counter % config.save_freq == 0:
                    for d_iter in range(self.disc_updates):
                        _d_opt, _d_sum, \
                        d_fk_loss, d_c_loss, \
                        d_rl_loss = self.sess.run([d_opt, self.g_sum,
                    ...
                    for i in range(3):
                        _ = self.sess.run([e_opt, de_opt, dl_opt])
                else:
                    for d_iter in range(self.disc_updates):
                        _d_opt, \
                        d_fk_loss, d_c_loss, \
                    ...
                    _g_opt, \
                    g_adv_loss, \
                    g_l1_loss = self.sess.run([g_opt, self.losses["g_adv_loss"][0],
                                               self.losses["g_l1_loss"][0]])
...
class SEAE(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        if gpu_idx == 0:
            ...
            #                                               self.canvas_size],
            #                                  name='sample_wavs')
            self.reference_G = self.generator(noisybatch, zvalue=class_val, 
            ...
            while not coord.should_stop():
                ...
                if (counter / num_devices) % config.save_freq == 0:
                    ...
                    for m in range(min(20, canvas_w.shape[0])):
                        ...
                        wavfile.write(os.path.join(save_path, 'sample_{}-{}.wav'.format(counter, m)), 8e3, canvas_w[m])
                        if not os.path.exists(os.path.join(save_path, 'gtruth_{}.wav'.format(m))):
                            wavfile.write(os.path.join(save_path, 'gtruth_{}.wav'.format(m)), 8e3, swaves[m])
                            wavfile.write(os.path.join(save_path, 'noisy_{}.wav'.format(m)), 8e3, sample_noisy[m])
                            wavfile.write(os.path.join(save_path, 'dif_{}.wav'.format(m)), 8e3, sample_dif[m])
